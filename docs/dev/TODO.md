...
- [ ] VDOT/Daniels 配速區間計算模組（含 Riegel 跨距離推算、成績時效性衰減處理）
- [ ] 週期化課表產生器（Base/Build/Peak/Taper，全馬 16–20 週框架）
- [ ] 課表產生器須支援**低至每週 3 次跑步**的配置，並確保 LSD 佔比與品質課配置合理
- [ ] 訓練負荷計算（ATL/CTL/TSB 概念，或對接 Garmin 既有 Training Load），需納入交叉訓練（重訓）造成的負荷
- [ ] 恢復狀態判斷邏輯（HRV/RHR/睡眠趨勢），需能處理欄位缺失時的降級判斷（呼應 1C 完整度標記）
- [ ] 支援**個人化恢復閾值**調校（例如「連續 N 天訓練即 HRV 明顯偏低」這類個別模式），而非只用通用規則
- [ ] **N-2**：支援匯入／協調外部課表（如跑團），至少要能標記某次訓練來自外部安排（設計方向待 P-1 確認）
- [ ] 支援**外部限制窗口**（旅遊／出差／賽事）：該區間自動降低強度或跳過，並調整前後週課表
